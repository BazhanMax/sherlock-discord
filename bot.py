...
@tree.command(
    name="search", guild=discord.Object(id=guild_id), description="search username"
...
async def slashcmd(interaction, username: str, timeout: int, nsfw: bool):
    ...
    Thread_for_search = Thread(
        target=sherFind, args=(my_queue, nsfw, username, timeout, tor)
    )
    ...
    await interaction.edit_original_response(
        content=f"[{username}]  found on [{len(results_list)}] sites:"
    )
